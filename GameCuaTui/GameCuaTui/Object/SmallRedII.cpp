#include "SmallRedII.h"

SmallRedII::SmallRedII(int x, int y) :BaseObject(SMALLREDII)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "smallred_II"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void SmallRedII::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	_sprite->render(spriteHandle, viewport);
}

void SmallRedII::update(float deltatime)
{
...
RECT SmallRedII::getBounding()
{
	return _sprite->getBounding();
}

float SmallRedII::checkCollision(BaseObject* object, float dt)
{
