...
BigBlueII::BigBlueII(int x, int y) :BaseObject(BIGBLUEII)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "bigblue_II"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void BigBlueII::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	_sprite->render(spriteHandle, viewport);
}

void BigBlueII::update(float deltatime)
{
...
RECT BigBlueII::getBounding()
{
	return _sprite->getBounding();
}

float BigBlueII::checkCollision(BaseObject* object, float dt)
{
