...
BigRedII::BigRedII(int x, int y) :BaseObject(BIGREDII)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "bigred_II"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void BigRedII::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	_sprite->render(spriteHandle, viewport);
}

void BigRedII::update(float deltatime)
{
...
RECT BigRedII::getBounding()
{
	return _sprite->getBounding();
}

float BigRedII::checkCollision(BaseObject* object, float dt)
{
