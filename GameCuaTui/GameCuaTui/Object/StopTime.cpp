...
StopTime::StopTime(int x, int y) :BaseObject(STOPTIME)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "stopwatch"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void StopTime::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	_sprite->render(spriteHandle, viewport);
}

void StopTime::update(float deltatime)
{
...
RECT StopTime::getBounding()
{
	return _sprite->getBounding();
}

float StopTime::checkCollision(BaseObject* object, float dt)
{
