#include "SmallBlueII.h"

SmallBlueII::SmallBlueII(int x, int y) :BaseObject(SMALLBLUEII)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "smallblue_II"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void SmallBlueII::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	_sprite->render(spriteHandle, viewport);
}

void SmallBlueII::update(float deltatime)
{
...
RECT SmallBlueII::getBounding()
{
	return _sprite->getBounding();
}

float SmallBlueII::checkCollision(BaseObject* object, float dt)
{
