...
Ball2::Ball2(int x, int y) :BaseObject(BALL2)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "ball_2"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void Ball2::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
...
RECT Ball2::getBounding()
{
	return _sprite->getBounding();
}

float Ball2::checkCollision(BaseObject* object, float dt)
{
