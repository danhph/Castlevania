...
Bone::Bone(int x, int y, int activeX) : BaseObject(BONE)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ENEMY);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ENEMY, "bone_1"));
	_sprite->setPosition(x, y);
	_sprite->setScale(2.0);
	_animation = new Animation(_sprite, 0.2f);
	_activeXLeft = activeX;
	_activeXRight = x;
	_animation->addFrameRect(eID::ENEMY, "bone_1", "bone_2",NULL);
...
void Bone::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	if (_hitPoint > 0)
...
void Bone::update(float deltatime)
{
	...
	if (_hitPoint > 0)
	{
		...
		{
			move->setVelocity(GVector2(BONE_MOVE_SPEED, 0));
...
void Bone::init()
{
	...
	auto movement = new Movement(GVector2(0, 0), GVector2(-BONE_MOVE_SPEED, 0), _sprite);
